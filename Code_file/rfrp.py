...
def record(pi, filename, name, rx_gpio, record_time_ms):
    ...
    if error:
        print(f"Max pulse limit ({MAX_PULSES}) exceeded. Recording saved with only {MAX_PULSES}.")
...
def main():
    parser = argparse.ArgumentParser(description="433 MHz ASK signal cloning")
    ...
    parser.add_argument("--time", type=int, default=DEFAULT_RECORD_MS, help="Recording time (ms)")
